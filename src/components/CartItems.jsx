                    ...
                    {all_products.map((e) => {
                        if (cartItems[e.id] > 0) {
                            return <tr key={e.id} className='border-b border-slate-900/20 p-6 medium-14 text-center'>
                                <td className='flexCenter '><img src={e.image} alt="prodctImg" height={43} width={43}
                                    className='rounded-lg rin-1 ring-slate-900/5 my-1' /></td>
                                <td><div className='line-clamp-3'>{e.name}</div></td>
                                <td>${e.new_price}</td>
                                <td className='w-16 h-16 bg-white'>{cartItems[e.id]}</td>
                               <td>${e.new_price * cartItems[e.id]}</td>
                               <td>
                                    <div className='bold-22 flexCenter'><TbTrash className='cursor-pointer' onClick={() => removeFromCart(e.id)} /></div>
                                </td>
                            </tr>
                        }
                        return null;
